...
export default function FunilPage() {
  ...
  const handleDragStart = (customer: CustomerWithSKUs) => {
    dragCustomer.current = customer
  ...
  const handleDrop = async (stage: PipelineStage) => {
    ...
    // Optimistic update
    setCustomers((prev) =>
      prev.map((c) => (c.id === customer.id ? { ...c, pipeline_stage: stage } : c))
    )
    setDragOverCol(null)
    ...
    if (err) {
      // Revert
      setCustomers((prev) =>
        prev.map((c) => (c.id === customer.id ? { ...c, pipeline_stage: customer.pipeline_stage } : c))
      )
    }
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Arraste os clientes entre as colunas para atualizar o estágio
        </p>
      </div>
        ...
        <div className="flex gap-4 min-w-max">
          {COLUMNS.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className={`flex flex-col w-72 rounded-2xl border ${col.borderColor} ${col.color} transition-all ${isOver ? 'ring-2 ring-[#1a8cff] scale-[1.01]' : ''}`}
                ...
                <div className="flex flex-col gap-2 p-3 min-h-32">
                  {colCustomers.map((customer) => {
                    ...
                    return (
                      <div
                        ...
                        className={`bg-[#111111] border border-[#2a2a2a] rounded-xl p-3 cursor-grab active:cursor-grabbing transition-all select-none ${
                          isDragging ? 'opacity-40 scale-95' : 'hover:border-[#1a8cff]/50'
                        }`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          <Link
                            ...
                            draggable={false}
                          >
                            <p className="font-semibold text-white text-sm truncate hover:text-[#1a8cff] transition-colors">
                              {customer.name}
                            </p>
                            {customer.phone && (
                              <p className="text-slate-400 text-xs mt-0.5 truncate">{customer.phone}</p>
                            )}
                            {skuCount > 0 && (
                          ...
                          {waNumber && (
                            <a
                              ...
                              onClick={(e) => e.stopPropagation()}
                              draggable={false}
                              className="flex-shrink-0 w-7 h-7 rounded-lg bg-green-900/40 hover:bg-green-800/60 text-green-400 flex items-center justify-center transition-colors"
                              title="WhatsApp"
                            >
                  ...
                  {colCustomers.length === 0 && (
                    <div className="flex items-center justify-center h-16 rounded-xl border border-dashed border-white/10">
                      <p className="text-slate-600 text-xs">Arraste um cliente aqui</p>
                    </div>
                  )}
                </div>
              </div>
