...
const NAV_ORDER = ['/clientes', '/clientes/customers', '/clientes/funil', '/clientes/follow-ups', '/clientes/tags']
...
export default function SwipeNavigation({ children }: { children: React.ReactNode }) {
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    if (Math.abs(dx) > 60 && Math.abs(dy) < 80 && !pathname.startsWith('/clientes/funil')) {
      const idx = NAV_ORDER.findIndex(p => p === pathname || (p !== '/clientes' && pathname.startsWith(p)))
      if (idx === -1) return
